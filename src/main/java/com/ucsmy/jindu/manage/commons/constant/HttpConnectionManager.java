 ...
 * Created by 
 * 
 * @author ucs_mojiazhou on 2017/5/4.
 */
@Component
public class HttpConnectionManager {

	protected PoolingHttpClientConnectionManager cm = null;
	protected PoolingHttpClientConnectionManager cms = null;
	protected SSLConnectionSocketFactory sslConnectionSocketFactory = null;
	...
	public void init() {
		...
		sslConnectionSocketFactory = new SSLConnectionSocketFactory(sslcontext, new String[] { "TLSv1", "SSLv3" }, null,
	...
	public CloseableHttpClient getHttpClient() {
		CloseableHttpClient httpClient = HttpClients.custom().setConnectionManager(cm).setDefaultRequestConfig(requestConfig)
				.setRetryHandler(this.requestRetryHandler()).build();
		return httpClient;
	}
	...
	public CloseableHttpClient getHttpsClient() {
		...
		CloseableHttpClient httpclient = HttpClients.custom().setSSLSocketFactory(sslConnectionSocketFactory).setConnectionManager(cms)
				.setDefaultRequestConfig(requestConfig).setRetryHandler(this.requestRetryHandler()).build();
		return httpclient;
	}
	...
	private HttpRequestRetryHandler requestRetryHandler() {
		...
		HttpRequestRetryHandler httpRequestRetryHandler = new HttpRequestRetryHandler() {
			public boolean retryRequest(IOException exception, int executionCount, HttpContext context) {
				...
				else if (exception instanceof NoHttpResponseException || exception instanceof InterruptedIOException
						|| exception instanceof ConnectTimeoutException) {// 如果服务器丢掉了连接，那么就重试
					return true;
				} else if (exception instanceof SSLHandshakeException || exception instanceof UnknownHostException
