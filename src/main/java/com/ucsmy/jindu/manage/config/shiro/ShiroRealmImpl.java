...
import com.ucsmy.jindu.manage.manage.service.impl.ManagePermissionServiceImpl;
...
public class ShiroRealmImpl extends AuthorizingRealm {
	...
	/*
	 * 
	 * @Autowired private ManagePermissionService managePermissionService;
	 */
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
		...
		if (!loginTypeConfig.getLocalType() && !token.isAutoLogin() && ucservice.isBindedOauthByUserId(token.getUsername())) {
		...
		return new SimpleAuthenticationInfo(new LoginUser(user.getUserId(), token.getUsername(), user.getAccount()), user.getPassword(), getName());
	}
