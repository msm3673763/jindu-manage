...
public class LogCommUtil {
    ...
    /**
     * 获取spring session里的ip信息
     * @return
     */
    public static String getSessionIpInfo() {
        Object requestAttributes = RequestContextHolder.getRequestAttributes();
        if (requestAttributes != null) {
            HttpServletRequest request = ((ServletRequestAttributes) requestAttributes).getRequest();
            HttpSession session = request.getSession();
            if (session != null) {
                return (String) session.getAttribute(MSG_IP_ADDRESS);
            }
        }
        return "";
    }
    ...
    public static ManageLogInfo getManageLogInfo(String info) {
        ...
        logInfo.setIpAddress(getSessionIpInfo());
        ShiroRealmImpl.LoginUser user = ShiroUtils.getContextUser();
        if (user != null) {
    ...
    private static String getParameterValue(Object parameter) {
        ...
        if (parameter instanceof String) {
            ...
            if(String.valueOf(parameter).indexOf("$") > 0) {
                parameter = String.valueOf(parameter).replaceAll("\\$", "\\\\\\$");
            }
